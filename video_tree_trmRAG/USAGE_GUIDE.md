...
4. [离线预处理：构建分层语义金字塔](#4-离线预处理构建分层语义金字塔)
   - 4.1 完整参数说明（含 Qwen 增强参数）
   - 4.2 典型使用场景
   - 4.3 **★ Qwen-VL 增强模式使用指南（v1.1）**
...
### 3.3 ★ Qwen-VL 增强模式（场景切分 + 关键帧筛选，v1.1）

```bash
# Qwen-VL 全增强：场景感知切分 + 关键帧精选 + Qwen 描述生成
python video_tree_trmRAG/build_pyramid.py \
  --video movie.mp4 \
  --vlm_backend qwen \
  --qwen_api_key "sk-0b74a6d774e145fc95d63c7a0c86b525" \
  --use_qwen_scene_detection \
  --scene_detection_probe_fps 0.5 \
  --scene_detection_batch_size 8 \
  --use_qwen_keyframe_scoring \
  --qwen_keyframe_keep_ratio 0.5 \
  --l2_duration 20 \
  --l3_fps 1.0 \
  --clip_device cuda
```

> 💡 **提示**：同一个 Qwen API key 同时用于场景检测、关键帧打分和 VLM 描述生成，无需额外配置。
...
| `--l1_duration` | float | 600.0 | L1 宏观事件段固定时长（秒，Qwen 场景检测开启时作参考值） |
...
| `--l3_max_frames` | int | 30 | 每 L2 片段最大 L3 帧数（Qwen 关键帧筛选后再截断） |
| `--max_frame_size` | int | 336 | 帧图像最大长边（像素） |
| `--vlm_backend` | str | `stub` | VLM 后端：`stub`/`ollama`/`openai`/`qwen` |
| `--ollama_url` | str | `http://localhost:11434/api/chat` | Ollama Chat API 地址 |
| `--vlm_model` | str | `llava` | VLM 模型名称（openai 时如 `gpt-4o`，qwen 时如 `qwen-vl-max`） |
| `--openai_api_key` | str | `""` | OpenAI API Key（`openai` 后端用） |
| `--qwen_api_key` | str | `""` | 阿里云百炼 API Key（`qwen` 后端及 Qwen 增强功能共用） |
...
**★ Qwen-VL 智能增强参数（v1.1 新增）**

| 参数 | 类型 | 默认值 | 说明 |
|------|------|-------|------|
| `--use_qwen_scene_detection` | 标志 | False | 启用 Qwen-VL 场景感知 L1 切分（替代固定步长），预期 +5-8% 准确率 |
| `--scene_detection_probe_fps` | float | 0.5 | 场景检测探测帧率（帧/秒），越高边界越精确，API 调用越多 |
| `--scene_detection_batch_size` | int | 8 | 每次发给 Qwen-VL 的探测帧数（相邻批次重叠 1 帧） |
| `--l1_min_scene_duration` | float | 60.0 | 过短 L1 段合并阈值（秒），避免碎片化 |
| `--l1_max_scene_duration` | float | 1200.0 | 过长 L1 段拆分阈值（秒），避免段过大 |
| `--use_qwen_keyframe_scoring` | 标志 | False | 启用 Qwen-VL L3 关键帧打分筛选，预期 +10-15% 准确率 |
| `--qwen_keyframe_keep_ratio` | float | 0.5 | 每 clip 内 L3 帧保留比例（0.0~1.0，如 0.5 = 保留一半） |
...
| 增强功能 | 解决的问题 | 预期提升 | 额外 API 调用量（2h 视频） |
|---------|-----------|---------|--------------------------|
| Qwen-VL 场景感知切分 | L1 段边界跨越场景，语义不连贯 | +5-8% Phase1 准确率 | ~515 次（0.5fps 探测） |
| Qwen-VL 关键帧筛选 | L3 层含大量低信息密度帧 | +10-15% Phase3 准确率 | ~N_clips 次（每 clip 1 次） |
| CLIP 视觉特征（保留） | 检索友好的跨模态向量空间 | 保持或略升 | 0（本地模型） |

**场景 D：仅启用关键帧筛选（推荐入门，API 调用量最小）**

关键帧筛选是性价比最高的增强：每个 clip 只需 1 次 API 调用，预期提升 10-15%。
...
python video_tree_trmRAG/build_pyramid.py \
  ...
  --scene_detection_probe_fps 0.5 \
  --scene_detection_batch_size 8 \
  --l1_min_scene_duration 60 \
  --l1_max_scene_duration 900 \
  --use_qwen_keyframe_scoring \
  --qwen_keyframe_keep_ratio 0.5 \
  --clip_device cuda
```

**场景 F：Qwen 增强 + 原有 Qwen 描述 + 本地 CLIP（混合最优）**

```bash
# 构建：Qwen 做切分+筛选+描述生成，CLIP 做视觉向量
python video_tree_trmRAG/build_pyramid.py \
  --video movie.mp4 \
  --vlm_backend qwen \
  --qwen_api_key "sk-0b74a6d774e145fc95d63c7a0c86b525" \
...
不传任何 `--use_qwen_*` 参数时，`build_pyramid.py` 与 v1.0 行为完全一致，不发起任何 Qwen API 调用，无需 `--qwen_api_key`。
...
| 参数 | 推荐值 | 说明 | 调整建议 |
|------|-------|------|---------|
| `l1_segment_duration` | 600s（10min） | L1 段固定时长（Qwen 切分时作参考） | 视频 < 30min 时可设为 180-300s |
| `l2_clip_duration` | 20s | L2 片段时长 | 快速动作视频可设为 5-10s |
| `l3_fps` | 1.0 | L3 帧率（候选帧采样率） | 高动态视频可设为 2.0，静止场景可设为 0.5 |
| `l1_max_frames_for_vlm` | 6 | L1 VLM 帧数 | 增大可提升摘要质量（但增加成本） |
| `l2_max_frames_for_vlm` | 4 | L2 VLM 帧数 | 同上 |
| `l3_max_frames_per_clip` | 30 | L3 帧数上限 | Qwen 筛选后再截断，可适当增大（如 60） |

**★ Qwen 增强参数调优（v1.1）**

| 参数 | 推荐值 | 说明 | 调整建议 |
|------|-------|------|---------|
| `use_qwen_scene_detection` | True（长视频） | 场景感知 L1 切分 | 视频 < 10min 无明显场景变化时关闭 |
| `scene_detection_probe_fps` | 0.5 | 探测帧率 | 动作片/综艺可设为 1.0；纪录片/讲座 0.2 即可 |
| `scene_detection_batch_size` | 8 | 每批帧数 | 增大可减少 API 调用，但降低边界精度 |
| `l1_min_scene_duration` | 60s | 合并过短段 | 剪辑频繁的 MV/广告可降至 30s |
| `l1_max_scene_duration` | 900s | 拆分过长段 | 单场景长的纪录片可放宽至 1800s |
| `use_qwen_keyframe_scoring` | True（推荐） | L3 关键帧精选 | 静态采访/课堂录像可关闭（场景变化少） |
| `qwen_keyframe_keep_ratio` | 0.5 | 帧保留比例 | 动作密集视频设 0.3（更激进筛选）；静止视频设 0.7 |
...
```yaml
# Video-Tree-TRM 配置文件（v1.1，含 Qwen-VL 增强参数）
...
pyramid:
  ...
  # ★ Qwen-VL 场景感知切分（v1.1，默认关闭）
  use_qwen_scene_detection: false
  scene_detection_probe_fps: 0.5      # 探测帧率 fps
  scene_detection_batch_size: 8       # 每批发送帧数
  l1_min_duration: 60.0              # 过短段合并阈值（秒）
  l1_max_duration: 1200.0            # 过长段拆分阈值（秒）

  # ★ Qwen-VL 关键帧筛选（v1.1，默认关闭）
  use_qwen_keyframe_scoring: false
  qwen_keyframe_keep_ratio: 0.5      # L3 帧保留比例
...
vlm:
  backend: "qwen"              # 选项: ollama, openai, qwen, stub
  # Qwen 后端配置（v1.1）
  qwen_api_key: "sk-0b74a6d774e145fc95d63c7a0c86b525"
...
### Q6（v1.1）：启用 `--use_qwen_scene_detection` 后报 `QwenSceneDetector` 未找到 API Key

**原因**：启用了场景检测但未提供 Qwen API Key。
...
### Q7（v1.1）：Qwen-VL 场景检测调用量超出预期

**原因**：`scene_detection_probe_fps` 过高，或视频过长。

**解决**：
```bash
# 降低探测帧率（每 5 秒 1 帧，API 调用减少 10×）
--scene_detection_probe_fps 0.2

# 或增大 batch_size（减少总批次数）
--scene_detection_batch_size 16
```

---

### Q8（v1.1）：Qwen 关键帧筛选后 L3 帧数为 0

**原因**：`qwen_keyframe_keep_ratio` 过低，且 clip 本身帧数极少（如只有 1-2 帧）。

**解决**：`QwenKeyframeScorer.filter_top_k` 内已有保底逻辑——`top_k = max(1, round(n * ratio))`，至少保留 1 帧。若仍为 0，检查 L3 帧提取是否成功（使用 `--verbose` 查看日志）。

---

### Q9：`FileNotFoundError: 视频文件不存在`
...
*文档版本：v1.1.0 | 最后更新：2026-03-04 | 新增：Qwen-VL 增强参数说明（§3.3/§4.1/§4.3）、YAML 配置更新、FAQ Q6-Q9*
